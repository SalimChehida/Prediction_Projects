...
water_flow_scaled = scaler.fit_transform(water_flow.values.reshape(-1, 1))

lookback = 24 * 7  # deux semaines
horizon = 7  # une semaine

X = []
y = []
for i in range(lookback, len(water_flow_scaled) - horizon + 1):
    X.append(water_flow_scaled[i - lookback:i])
    y.append(water_flow_scaled[i:i + horizon])
X = np.array(X)
y = np.array(y)
...
# Dates correspondant aux données de test prédites
dates_test = dates[train_size + lookback:train_size + lookback + len(y_pred)]
